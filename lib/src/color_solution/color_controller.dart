...
import 'color_box/color_box_controller.dart';

class ColorController extends GetxController with GetSingleTickerProviderStateMixin {
  ...
  void reset() {
    ...
    for (BoxModel m in _initBoxs) {
      BoxModel newModel = BoxModel.fromJson(m.toJson());
      boxs.add(newModel);
      Get.find<ColorBoxController>().update([newModel.id!]);
    }
  }
  ...
  void solved() {
    ...
    Get.find<ColorBoxController>().update(boxs.map((e) => e.id!).toList());
  ...
  void sync() {
    ...
    Get.toNamed(Routes.SOLUTION, arguments: cube);
  }
